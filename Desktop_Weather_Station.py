    ...
    while go:
        try:
            ...
            scroll_display(datetime.datetime.now().strftime('%H:%M') + 
                           datetime.datetime.now().strftime(' %m-%d-%y'),1,LED_orientation,0.07)                #Read and display time and date
            if show_metric:
                scroll_display('{0:.1f}'.format(temperature)+chr(176)+'C',orange,LED_orientation,0.07)          #Display temperature (Centigrade)
            else:
                scroll_display('{0:.1f}'.format(1.8*temperature+32)+chr(176)+'F',orange,LED_orientation,0.07)   #Display temperature (Farenheit)
            if relative_humidity > 40 and relative_humidity < 60: color=green
            else: color=red
            scroll_display('{0:.0f}'.format(relative_humidity)+'% ',color,LED_orientation,0.07)                 #Display relative humidity (%)
